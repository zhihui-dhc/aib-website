...
function writePostsIndexFile (data) {
  let file = '<!--THIS FILE IS GENERATED BY `./blog/build.js` DO NOT EDIT HERE-->\n'
  file += '<template>\n'
  file += '  <div class="header-padding"></div>\n'
  file += '  <div class="article-wrapper" id="blog-index">\n'
  file += `    <h1>${blogTitle}</h1>\n`
  file += '    <div class="subtitle"><a href="/static/feed.xml">Subscribe</a> for the latest updates.</div>\n'
  for (let i = 0; i < data.length; i++) {
    file += `    <a class="article-link" v-link="{ path: '/blog/${data[i].slug}'}">\n`
    file += `      <div class="article-title">${data[i].title}</div>\n`
    file += `      <div class="article-excerpt">${data[i].excerpt}</div>\n`
    file += `      <div class="article-date">Posted on ${data[i].dateFriendly}</div>\n`
    file += '    </a>\n'
  } 
  file += '  </div><!--article-wrapper-->\n'
  file += '</template>'

  fs.writeFileSync(blogIndexFile, file, 'utf8')

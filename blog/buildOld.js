...
function writePostsIndexFile (data) {
  let f = '<!--THIS FILE IS GENERATED BY `./blog/build.js` DON\'T EDIT HERE-->\n'
  f += '<template>\n'
  f += '  <div>\n'
  ...
  f += '    title () {\n'
  f += '      return {\n'
  f += '        inner: this.$t(\'siteBlog.title\'),\n'
  f += '        separator: \'-\',\n'
  f += '        complement: \'Cosmos - \' + this.$t(\'site.networkOfBlockchains\')\n'
  f += '      }\n'
  f += '    }\n'
  f += '  }\n'
  f += '}\n'
...
function writeRoutesFile (data) {
  ...
  let file = '// THESE ROUTES ARE GENERATED BY `./blog/build.js` DON\'T EDIT HERE-->\n'
  file += 'var blogRoutes = [\n'
  for (let i = 0; i < data.length; i++) {
    let routePath = `'/blog/${data[i].slug}'`
    let routeComponent =  `post('${data[i].slug}')`
